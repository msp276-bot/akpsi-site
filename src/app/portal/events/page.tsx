...
import {
  CalendarPlus, Check, ChevronLeft, ChevronRight, Clock, ExternalLink,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="inline-flex rounded-lg border border-line bg-white p-1">
            {[["month", LayoutGrid, "Month"], ["week", CalendarPlus, "Week"], ["list", List, "List"]].map(([key, Icon, label]) => <button key={key as string} onClick={() => setView(key as CalendarView)} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium ${view === key ? "bg-navy text-white" : "text-muted"}`}><Icon size={15} /> {label as string}</button>)}
          </div>
        </div>
      </div>
